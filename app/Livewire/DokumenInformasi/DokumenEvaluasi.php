...
/**
 * @var \Illuminate\Pagination\LengthAwarePaginator $evaluasi
 * @property \Illuminate\Contracts\Pagination\LengthAwarePaginator $evaluasi
 */

class DokumenEvaluasi extends Component
{
    use WithPagination;
    ...
    {
        $this->tahuns = DokumenEvaluasiModel::select('tahun')
            ->distinct()
            ->orderByDesc('tahun')
            ->pluck('tahun')
            ->toArray();
    ...
    public function updatedSearchPage()
    {
        ...
        if ($page >= 1 && $page <= $this->getDokumenEvaluasiProperty()->lastPage()) {
            $this->gotoPage($page);
    ...
    {
        $this->detailData = DokumenEvaluasiModel::findOrFail($id);
    ...
    {
        $evaluasis = DokumenEvaluasiModel::findOrFail($id);
        $evaluasis->increment('downloads');

        if (Str::startsWith($evaluasis->file_path, ['http://', 'https://'])) {
            return redirect()->away($evaluasis->file_path);
        }

        $path = storage_path('app/public/' . $evaluasis->file_path);
        if (!file_exists($path)) {
    ...
    protected function getDokumenEvaluasiProperty()
    {
        $query = DokumenEvaluasiModel::query();
    ...
    {
        $evaluasis = $this->getDokumenEvaluasiProperty();
        return view('livewire.dokumen-informasi.dokumen-evaluasi', [
            'evaluasis' => $evaluasis,
            'total' => $evaluasis->total(),
            'lastPage' => $evaluasis->lastPage(),
            'currentPage' => $evaluasis->currentPage(),
