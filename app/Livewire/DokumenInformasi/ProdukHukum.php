...
/**
 * @var \Illuminate\Pagination\LengthAwarePaginator $produk
 * @property \Illuminate\Contracts\Pagination\LengthAwarePaginator $produk
 */

class ProdukHukum extends Component
{
    use WithPagination;
    ...
    {
        $this->tahuns = ProdukHukumModel::select('tahun')
            ->distinct()
            ->orderByDesc('tahun')
            ->pluck('tahun')
            ->toArray();
    ...
    public function updatedSearchPage()
    {
        ...
        if ($page >= 1 && $page <= $this->getProdukHukumProperty()->lastPage()) {
            $this->gotoPage($page);
    ...
    {
        $this->detailData = ProdukHukumModel::findOrFail($id);
    ...
    {
        $produks = ProdukHukumModel::findOrFail($id);
        $produks->increment('downloads');

        if (Str::startsWith($produks->file_path, ['http://', 'https://'])) {
            return redirect()->away($produks->file_path);
        }

        $path = storage_path('app/public/' . $produks->file_path);
        if (!file_exists($path)) {
    ...
    protected function getProdukHukumProperty()
    {
        $query = ProdukHukumModel::query();
    ...
    {
        $produks = $this->getProdukHukumProperty();
        return view('livewire.dokumen-informasi.produk-hukum', [
            'produks' => $produks,
            'total' => $produks->total(),
