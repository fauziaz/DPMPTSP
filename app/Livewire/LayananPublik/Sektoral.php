...
/**
 * @var \Illuminate\Pagination\LengthAwarePaginator $sektoral
 * @property \Illuminate\Contracts\Pagination\LengthAwarePaginator $sektoral
 */

class Sektoral extends Component
{
    use WithPagination;
    ...
    {
        $this->tahuns = SektoralModel::select('tahun')
            ->distinct()
            ->orderByDesc('tahun')
            ->pluck('tahun')
            ->toArray();
    ...
    public function updatedSearchPage()
    {
        ...
        if ($page >= 1 && $page <= $this->getSektoralProperty()->lastPage()) {
            $this->gotoPage($page);
    ...
    {
        $this->detailData = SektoralModel::findOrFail($id);
    ...
    {
        $sektorals = SektoralModel::findOrFail($id);
        $sektorals->increment('downloads');

        if (Str::startsWith($sektorals->file_path, ['http://', 'https://'])) {
            return redirect()->away($sektorals->file_path);
        }

        $path = storage_path('app/public/' . $sektorals->file_path);
        if (!file_exists($path)) {
    ...
    protected function getSektoralProperty()
    {
        $query = SektoralModel::query();
    ...
    {
        $sektorals = $this->getSektoralProperty();
        return view('livewire.layanan-publik.sektoral', [
            'sektorals' => $sektorals,
            'total' => $sektorals->total(),
            'lastPage' => $sektorals->lastPage(),
            'currentPage' => $sektorals->currentPage(),
