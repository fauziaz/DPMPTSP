<div class="profil-pimpinan-page">
    ...
    <x-template.hero-section
        ...
        :breadcrumbs="['Beranda' => '/']"
        title="IKM"
        subtitle="Temukan Indeks Kepuasan Masyarakat DPMPTSP Kota Tasikmalaya"
    />
                        ...
                        @if($ikms->count())
                            @foreach($ikms as $ikm)
                                <x-dokumen-informasi.dokumen-card 
                                    :judul="$ikm->judul"
                                    :deskripsi="$ikm->deskripsi"
                                    label="IKM"
                                    :downloads="$ikm->downloads"
                                    tanggal="{{ \Carbon\Carbon::parse($ikm->tanggal)->locale('id')->translatedFormat('l, d F Y') }}"
                                    :id="$ikm->id"
                                    :file_path="$ikm->file_path"
                                    :iconClass="$this->getIconClass($ikm->file_path)"
                                />
                            @endforeach
                        @else
